...
def realize_mineral(mineral):
    ...
    mineral.params['grueneisen_0'] = mineral.params[
        'grueneisen_0'] + normal(scale=mineral.uncertainties['err_grueneisen_0'])
...
if whattodo == "plotgood" and len(sys.argv) > 2:
    ...
    for name in names:
        if name.endswith(".n") or name.endswith(".V_0") or name.endswith(
                ".molar_mass") or name.endswith(".F_0") or name.endswith(".P_0"):
            i += 1
            continue
        plt.subplot(5, 8, idx)
        idx += 1
    ...
    error = np.sum([err_rho / np.mean(seis_rho), err_vphi /
                    np.mean(seis_vphi), err_vs / np.mean(seis_vs)])
    ...
    for i in range(n_realizations):
        ...
        try:
            ...
            error = np.sum([err_rho /
                            np.mean(seis_rho), err_vphi /
                            np.mean(seis_vphi), err_vs /
                            np.mean(seis_vs)])
    ...
    values = [
        ...
        2,
        -1.6065263588976038,
        7.5954915681374134e-05,
        9.6441602176002807e-07,
        4.4326026287552629e-05,
        3.0664473372061482e-05]
    names = [
        'anchor_T',
        "'burnman.minerals.SLB_2011_ZSB_2013.mg_perovskite'.Gprime_0",
        "'burnman.minerals.SLB_2011_ZSB_2013.mg_perovskite'.K_0",
    ...
    c = matplotlib.colors.LinearSegmentedColormap.from_list('vphi', [(0, '#ffffff'), (0.2, '#eff3ff'), (
        0.4, '#bdd7e7'), (0.6, '#6baed6'), (0.8, '#3182bd'), (1.0, '#08519c')], gamma=gamma)
    ...
    c = matplotlib.colors.LinearSegmentedColormap.from_list('vphi', [(0, '#ffffff'), (0.2, '#fee5d9'), (
        0.4, '#fcae91'), (0.6, '#fb6a4a'), (0.8, '#de2d26'), (1.0, '#a50f15')], gamma=gamma)
    c.set_bad('w', alpha=1.0)
    ...
    c = matplotlib.colors.LinearSegmentedColormap.from_list('vphi', [(0, '#ffffff'), (0.2, '#edf8e9'), (
        0.4, '#bae4b3'), (0.6, '#74c476'), (0.8, '#31a354'), (1.0, '#006d2c')], gamma=gamma)
    c.set_bad('w', alpha=1.0)

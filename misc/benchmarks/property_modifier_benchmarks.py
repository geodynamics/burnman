...
import burnman
from burnman import minerals, constants
import numpy as np
...
P = 1.e5
temperatures = np.linspace(400., 1500., 101)
C_ps = np.empty_like(temperatures)
Qs = np.empty_like(temperatures)
Hs = np.empty_like(temperatures)
for i, T in enumerate(temperatures):
...
ab.property_modifiers = [['bragg_williams',
                          {'deltaH': 14000.0,
                           'deltaV': 4.2e-07,
                           'Wh': 13600.0,
                           'Wv': 4.2e-07,
...
P = 1.e5
temperatures = np.linspace(400., 1200., 101)
C_ps = np.empty_like(temperatures)
Qs = np.empty_like(temperatures)
Hs = np.empty_like(temperatures)
for i, T in enumerate(temperatures):
...
ax[0].imshow(img_ab_Cp, extent=[400.0, 1100.0, 0., 120.0], aspect='auto')  # paper * 2
img_ab_enthalpy_Q = mpimg.imread('figures/albite_disordering_enthalpy_Q.png')
ax[1].imshow(img_ab_enthalpy_Q, extent=[450.0, 1250.0, 0., 1.0], aspect='auto')
ax[2].imshow(img_ab_enthalpy_Q, extent=[450.0, 1250.0, 0., 15.0], aspect='auto')

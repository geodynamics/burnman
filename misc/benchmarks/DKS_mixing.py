...
pressures = np.array([0.0, 5.0, 10.0, 25.0, 50.0, 75.0, 100.0, 135.0]) * 1.0e9
temperatures = np.array(
    [3000.0, 3000.0, 3000.0, 3000.0, 4000.0, 5000.0, 6000.0, 6000.0]
)

SiO2_enthalpies, SiO2_entropies, SiO2_volumes = phases[0].evaluate(
    ["H", "S", "V"], pressures, temperatures
)
MgO_enthalpies, MgO_entropies, MgO_volumes = phases[-1].evaluate(
    ["H", "S", "V"], pressures, temperatures
)
...
for phase in phases[1:-1]:
    ...
    enthalpies, entropies, volumes = phase.evaluate(
        ["H", "S", "V"], pressures, temperatures
    )

    excess_enthalpies = enthalpies / sum_cations - (
        fSi * SiO2_enthalpies + (1.0 - fSi) * MgO_enthalpies
    )
    excess_entropies = entropies / sum_cations - (
        fSi * SiO2_entropies + (1.0 - fSi) * MgO_entropies
    )
    excess_volumes = volumes / sum_cations - (
        fSi * SiO2_volumes + (1.0 - fSi) * MgO_volumes
    )
...
for pressure, temperature in zip(*[pressures, temperatures]):
    mask = [
        ...
        if np.abs(excesses[i][0] - pressure) < 1.0
        and np.abs(excesses[i][1] - temperature) < 1.0
    ]
    ax_H.plot(
        excesses[mask, 2],
        excesses[mask, 3],
        marker="o",
        linestyle="None",
        label="{0:.0f} GPa, {1:.0f} K".format(pressure / 1.0e9, temperature),
    )
    ax_S.plot(
        excesses[mask, 2],
        excesses[mask, 4],
...
mask = [i for i in range(len(excesses[:, 2])) if np.abs(excesses[i][2] - 0.5) < 1.0e-12]
print("P (GPa)   T (K)   H (J/mol)   S (J/K/mol)   V (cm^3/mol)")
for excess in excesses[mask]:
    print(
        "{0:.0f} {1:.0f} {2:.1e} {3:.1e} {4:.1e}".format(
            excess[0] / 1.0e9, excess[1], excess[3], excess[4], excess[5]
        )
    )

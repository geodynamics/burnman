...
def check_mgd_shim_duffy_kenichi():
    ...
    ref_pressures = [np.array([0.,
                               ...
                               170.38,
                               198.07])]
    ref_pressures.append(np.array([4.99,
                                   8.53,
                                   12.53,
                                   17.04,
                                   22.13,
                                   ...
                                   175.24,
                                   202.90]))
    ref_pressures.append(np.array([12.14,
                                   15.69,
                                   19.68,
                                   24.19,
                                   29.28,
                                   ...
                                   182.36,
                                   210.02]))
    ref_pressures.append(np.array([19.30,
                                   22.84,
                                   26.84,
                                   31.35,
                                   36.44,
...
def check_slb_fig7_txt():
    ...
    data = np.loadtxt(
        "../../burnman/data/input_minphys/slb_fig7.txt",
        skiprows=2)
    ...
    for i in range(len(temperature)):
        ...
        gibbs_comp[i] = 100. * (forsterite.molar_gibbs /
                                1.e6 - gibbs[i]) / gibbs[i]
        entropy_comp[i] = 100. * (forsterite.molar_entropy -
                                  entropy[i]) / (entropy[i] if entropy[i] != 0. else 1.)
        enthalpy_comp[i] = 100. * (forsterite.molar_enthalpy / 1.e6 -
                                   enthalpy[i]) / (enthalpy[i] if enthalpy[i] != 0. else 1.)
...
def check_averaging():
    ...
    # it here
    porosity = np.array([0.0, 0.05, 0.11, 0.13, 0.25,
                         0.33, 0.36, 0.39, 0.44, 0.50, 0.70])
    berryman_bulk_modulus = np.array([46.3,
                                      41.6,
                                      36.6,
                                      35.1,
                                      27.0,
...
def check_averaging_3():
    ...
    for i in range(len(volumes)):
        ...
        hs_av_bulk_modulus[i] = 0.5 * \
            hsl_bulk_modulus[i] + 0.5 * hsu_bulk_modulus[i]
        hs_av_shear_modulus[i] = 0.5 * \
            hsl_shear_modulus[i] + 0.5 * hsu_shear_modulus[i]

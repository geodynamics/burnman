...
for database, f, mineral in filemin:
    f = open(f, 'r')
    datalines = [line.strip() for idx, line in enumerate(
    ...
    print(
        'Benchmarks for',
        database,
        'database with method',
        fo.params['equation_of_state'])
    print(variables)
    ...
    for line in data:
        ...
        gibbs = H - T * S
        PT.append([P / 1.e4, T])
        diff = [p(fo.gibbs, gibbs), p(fo.H, H), p(fo.S, S), p(fo.V, V /
                                                              1.e5), p(fo.C_p, C_p), p(fo.alpha, alpha), p(fo.K_T, 1.e5 /
                                                                                                           beta), p(fo.density, rho)]
        print(diff)
...
perplex_output = [[1., 4.3660, 0.77818E-06, 3222.4], [50000., 4.2104,
                                                      0.67868E-06, 3341.5], [100000., 4.0778, 0.60406E-06, 3450.2]]
T = 298.15
...
def magnetic_gibbs(T, Tc, beta, p):
    ...
    tau = T / Tc
    if tau < 1:
        f = 1. - (1. / A) * (79. / (140. * p * tau) + (474. / 497.) * (1. / p - 1.) *
                             (np.power(tau, 3.) / 6. + np.power(tau, 9.) / 135. + np.power(tau, 15.) / 600.))
    else:
        f = -(1. / A) * (np.power(tau, -5) / 10. +
                         np.power(tau, -15) / 315. + np.power(tau, -25) / 1500.)
    return constants.gas_constant * T * np.log(beta + 1.) * f


def HSERFe(T):
    if T < 1811:
        gibbs = 1224.83 + 124.134 * T - 23.5143 * T * \
            np.log(T) - 0.00439752 * T * T - \
            5.89269e-8 * T * T * T + 77358.3 / T
    else:
        gibbs = -25384.451 + 299.31255 * T - 46. * T * \
            np.log(T) + 2.2960305e31 * np.power(T, -9.)
    return gibbs
...
def gibbs_fcc_1bar(T):
    ...
    p = 0.28
    if T < 1811:
        gibbs = HSERFe(T) - 1462.4 + 8.282 * T - 1.15 * \
            T * np.log(T) + 0.00064 * T * T
    else:
        gibbs = - 27098.266 + 300.25256 * T - 46. * T * \
            np.log(T) + 2.78854e31 * np.power(T, -9.)
...
plt.plot(
    temperatures,
    np.array(fcc_gibbs_model) -
    np.array(bcc_gibbs_model),
    'b-',
    linewidth=1.,
    label='FCC-BCC model')

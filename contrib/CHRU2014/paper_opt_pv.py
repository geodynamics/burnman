...
import burnman
from contrib.CHRU2014.helper_solid_solution import (
    SLB_2011_ZSB_2013_mg_fe_perovskite,
    other_ferropericlase,
)
...
if __name__ == "__main__":
    ...
    number_of_points = (
        20  # set on how many depth slices the computations should be done
    )
    ...
    #    plt.figure(dpi=100,figsize=figsize)
    plt.subplot(2, 2, 2)
    plt.plot(seis_p / 1.0e9, seis_vs * 0.0, color="k", linestyle="-", linewidth=2.0)
    plt.plot(
        seis_p / 1.0e9,
    ...
    plt.plot(
        seis_p / 1.0e9,
        (A_vphi - seis_vphi) / seis_vphi * 100.0,
        color=colors.color(3),
        markevery=5,
        marker="s",
        markersize=5,
    ...
    plt.plot(
        seis_p / 1.0e9,
        (B_vphi - seis_vphi) / seis_vphi * 100.0,
        color=colors.color(1),
        markevery=5,
        marker="s",
        markersize=5,
    ...
    plt.plot(
        seis_p / 1.0e9,
        (C_vphi - seis_vphi) / seis_vphi * 100.0,
        color=colors.color(4),
        markevery=5,
        marker="s",
        markersize=5,

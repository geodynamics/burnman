...
if __name__ == "__main__":
    ...
    pMg1s = np.linspace(0.0, 1.0, 201)
    pMg2s = np.linspace(0.0, 1.0, 201)

    cen.set_state(1.0e5, 100.0)
    cfs.set_state(1.0e5, 100.0)
    ...
    for i in range(201):
        ...
        for j in range(201):
            ...
            p_Mg = (pMg1s[i] + pMg2s[j]) / 2.0
            H_base = H_cfs * (1.0 - p_Mg) + H_cen * p_Mg

            if True:
                for px_idx, px in enumerate(pyroxenes):
                    px.set_composition([p_fs, p_en, p_fm])
                    px.set_state(1.0e5, 100.0)
    ...
    lvl1 = np.linspace(-4000.0, 2000.0, 13)
    ...
    lbl = [
        f"a) $\\mathcal{{G}}$* ({pyroxenes[0].name})",
    ...
    for i in range(4):
        (disordered_line,) = ax[i].plot(
            [0.0, 1.0], [0.0, 1.0], color="black", zorder=100
        )
    ...
    for j_px in [0, 1, 2]:  # disordered, convergent and non convergent cases
        for k, T in enumerate([400.0, 500.0]):  # temperatures
            for i, x_Fe in enumerate(x_Fes):
                equilibrate_px(
                    pyroxenes[j_px], x_Fe=x_Fe, P=1.0e8, T=T
                )  # none of the models are pressure dependent.
                pMg1s[i] = pyroxenes[j_px].molar_fractions[0]
                pMg2s[i] = 1.0 - pyroxenes[j_px].molar_fractions[1]

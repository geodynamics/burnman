...
ol_1bar_lattice_data_Suzuki = np.loadtxt(
    'data/Suzuki_1975_ol_Kenya_expansion.dat')
...
if run_fitting:
    ...
    sol = minimize(orthorhombic_misfit, guesses, method='Nelder-Mead',
                   args=[[i, min]], options={'rhobeg': 0.2, 'maxiter': 100000})
    print(sol)
...
if do_plotting:
    ...
    print_table_for_mineral_constants_2(m, ['a', 'b_1', 'c_1',
                                            'b_2', 'c_2', 'd'],
                                        [(1, 1), (2, 2), (3, 3),
                                         (1, 2), (1, 3),
                                         (4, 4), (5, 5), (6, 6)])
    ...
    Vthird_expansion = 1.e4 * \
        (np.power(np.prod(extensions*1.e-4 + 1, axis=1), 1./3.) - 1.)
    ...
    Vthird_expansion = 1.e4 * \
        (np.power(np.prod(ol_1bar_lattice_data_Suzuki[:, 1:4]*1.e-4 + 1,
                          axis=1), 1./3.) - 1.)
    ax[1].scatter(ol_1bar_lattice_data_Suzuki[:, 0]+273.15,
                  Vthird_expansion,
    ...
    for T in temperatures:
        ...
        data_mask = np.array(
            [i for i, d in enumerate(ol_data) if np.abs(d[0] - T) < 1])
        ...
        if False:
            for i, (p, q) in enumerate(i_pq):
                ...
                # color=ln[0].get_color())
                ax[i].scatter(np.log(rho0/1000./ol_data[data_mask, 2]),
                              SNoverbetaN_data[data_mask, p-1, q-1],
                              color=ln[0].get_color())
        else:
            for i, (p, q) in enumerate(i_pq):
                ln = ax[i].plot(
                    pressures/1.e9, C[:, p-1, q-1]/1.e9, label=f'{T} K')
                j = 4 + 2*i
                ax[i].scatter(T_data[:, 1], T_data[:, j],
                              color=ln[0].get_color())

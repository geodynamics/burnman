...
def print_table_for_mineral_constants_2(mineral, param_list, indices):
    ...
    rows = [['$p$', '$q$']]
    rows[0].extend([f'${param.split("_")[0]}_{{{param.split("_")[1]}pq}}$' for param in param_list])
    for ci, (i, j) in enumerate(indices):
        row = [f'{i}', f'{j}']
        row.extend([f'{constants[ci, i]:.4e}' if constants[ci, i] != 0 and constants[ci, i] != 1 else '-' for i in range(len(param_list))])
        rows.append(row)

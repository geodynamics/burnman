...
change with pressure and temperature.  In this step we explore how
uncertainties in these parameters might affect the conclusions you draw.
...
look at how perturbations to :math:`K_{0}^{'}` and :math:`G_{0}^{'}`
(the pressure derivatives of the
...
if __name__ == "__main__":
    ...
    parameters, then returns the modified mineral.
    Typical values for :math:`K_{0}^{'}` are around 4 and typical
    values for :math:`G_{0}^{'}` are around 2
    ...
    and save the seismic profiles for later visualization.
    """
    ...
    for i in range(n_realizations):
        ...
        # If that happens, we simply disregard this realization of the rock.
        except ValueError:
    ...
    intensity corresponds to how many of the models went through that
    portion of the velocity/density space.
    The dashed lines show the PREM values. Tighter histograms indicate that the
    perturbations to the mineral physical values make less of a difference,
    more spread out ones show larger effects.

    In general, this plotting is more complex than the ones
    in step 1 and step 2 and you may safely ignore it.
    """

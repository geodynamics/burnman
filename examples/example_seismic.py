...
if __name__ == "__main__":
        ...
        for m in range(len(models)):
            ...
            try:
                ...
                plt.plot(
                    ...
                    color=colors[m],
                    linestyle='-',
                    label=models[m].__class__.__name__)
            except:
                # write out warning that the variable failed for given model
                print(
                    var[a] +
                    ' is not defined for ' +
                    models[m].__class__.__name__)
    ...
    pressure, gravity, v_p, v_s, v_phi, density = models[0].evaluate(
        [
            'pressure', 'gravity', 'v_p', 'v_s', 'v_phi', 'density'], models[0].internal_depth_list(
            mindepth=-1.e3, maxdepth=6372.1e3))

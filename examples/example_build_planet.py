...
if __name__ == "__main__":
    ...
    class Mercury(object):
        ...
        def __init__(self, n_slices):
            ...
            # initial guess at pressure profile
            self.pressures = np.linspace(35.0e9, 0.0, n_slices)
            # Assume an isothermal interior (not a great assumption, but we do
            # this for simplicity's sake).
            self.temperatures = np.ones_like(self.pressures) * 1000.
        ...
        def _evaluate_eos(self, pressures, temperatures, radii):
            ...
            for i in range(len(radii)):
                ...
                if radii[i] > self.cmb:
                    density, vs, vphi = self.mantle.evaluate(['density', 'v_s', 'v_phi'], [
                                                             pressures[i]], [temperatures[i]])
                else:
                    density, vs, vphi = self.core.evaluate(['density', 'v_s', 'v_phi'], [
                                                           pressures[i]], [temperatures[i]])
        ...
        def _compute_gravity(self, density, radii):
            ...
            # Set it to zero a the center, since radius = 0 there we cannot
            # divide by r^2
            grav[0] = 0.0
            return grav
    ...
    ax1.plot(
        merc.radii / 1.e3,
        merc.bulk_sound_speed / 1.e3,
        label=r'$V_\phi$',
        linewidth=2.)
    ax1.plot(
        merc.radii / 1.e3,
        merc.shear_velocity / 1.e3,
        label=r'$V_S$',
        linewidth=2.)

...
if __name__ == "__main__":
    ...
    # Here we define the composition and the assemblage of interest
    ...
    # three phases coexist. The gibbs minimizer takes compositional
    ...
    # Here we call the gibbs minimizer. The output is given as a list, where
    # pressure and temperature are always the first two elements,
    # followed by the compositional vector
    sol = gibbs_minimizer(composition, assemblage, constraints)
    P_inv = sol['P']
    T_inv = sol['T']

    # Here we simply print the P-T position of the invariant point
    ...
    # Finally, let's plot the full phase diagram
    ...
    # To solve for the positions of the reaction lines, we need to change the assemblages
    # and constraints, as only two phases are stable.
    for i, P in enumerate(lo_pressures):
        constraints= [['P', P], ['X', [1., 0.], [1., 1.], 0.]]
    
        assemblage = burnman.Composite([andalusite, kyanite])
        and_ky_temperatures[i] = gibbs_minimizer(composition, assemblage, constraints)['T']
        
        assemblage = burnman.Composite([andalusite, sillimanite])
        and_sill_temperatures[i] = gibbs_minimizer(composition, assemblage, constraints)['T']
    
    for i, P in enumerate(hi_pressures):
        constraints= [['P', P], ['X', [1., 0.], [1., 1.], 0.]]
        
        assemblage = burnman.Composite([sillimanite, kyanite])
        sill_ky_temperatures[i] = gibbs_minimizer(composition, assemblage, constraints)['T']
    ...
    # The output of the bulk minimizer is a list containing P, T, X

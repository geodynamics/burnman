...
if __name__ == "__main__":
    ...
    print(
        "Bulk modulus bounds: {0:.3e} {1:.3e} {2:.3e}".format(
            basalt.isentropic_bulk_modulus_reuss,
            basalt.isentropic_bulk_modulus_vrh,
            basalt.isentropic_bulk_modulus_voigt,
        )
    )
    print(
        "Shear modulus bounds: {0:.3e} {1:.3e} {2:.3e}".format(
            basalt.isentropic_shear_modulus_reuss,
            basalt.isentropic_shear_modulus_vrh,
            basalt.isentropic_shear_modulus_voigt,
        )
    )
    print(
        "Universal elastic anisotropy: {0:.4f}\n"
        "Isotropic poisson ratio: {1:.4f}\n".format(
            basalt.isentropic_universal_elastic_anisotropy,
    ...
    nu_100_010 = basalt.isentropic_poissons_ratio(
    ...
    print(
        "Bulk modulus bounds: {0:.3e} {1:.3e} {2:.3e}".format(
            talc.isentropic_bulk_modulus_reuss,
    ...
    print(
        "Shear modulus bounds: {0:.3e} {1:.3e} {2:.3e}".format(
            talc.isentropic_shear_modulus_reuss,
            talc.isentropic_shear_modulus_vrh,
            talc.isentropic_shear_modulus_voigt,
        )
    )
    print(
        "Universal elastic anisotropy: {0:.3f}\n"
        "Isotropic poisson ratio: {1:.3f}".format(
            talc.isentropic_universal_elastic_anisotropy,
    ...
    def plot_anisotropic_seismic_properties(mineral):
        ...
        zeniths = np.linspace(np.pi / 2.0, np.pi, 31)
        azimuths = np.linspace(0.0, 2.0 * np.pi, 91)
        ...
        for i, item in enumerate(items):
            ...
            im.append(
                ax[i].contourf(
                    theta, r, item, ndivs, cmap=plt.cm.jet_r, vmin=vmin, vmax=vmax
                )
            )
            lines = ax[i].contour(
                theta, r, item, ticks, colors=("black",), linewidths=(1,)
            )

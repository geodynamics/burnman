...
)  # pick a seismic model, see seismiic.py for details
number_of_points = 20  # set on how many depth slices the computations should be done
depths = np.linspace(750.0e3, 2890.0e3, number_of_points)
...
def calc_velocities(ref_rho, K_0, K_prime, G_0, G_prime):
    ...
    mat_rho, mat_vphi, mat_vs = rock.evaluate(
...
if __name__ == "__main__":
    ...
    ref_rho = pymc.Uniform("ref_rho", lower=3300.0, upper=4500.0)
    K_0 = pymc.Uniform("K_0", lower=200.0e9, upper=300.0e9)
    K_prime = pymc.Uniform("Kprime_0", lower=3.0, upper=6.0)
    G_0 = pymc.Uniform("G_0", lower=50.0e9, upper=250.0e9)
    G_prime = pymc.Uniform("Gprime_0", lower=0.0, upper=3.0)
    ...
    def theta(p1=ref_rho, p2=K_0, p3=K_prime, p4=G_0, p5=G_prime):
        ...
        try:
            ...
            if e < minerr:
                minerr = e
                print("best fit", e, "values:", p1, p2 / 1.0e9, p3, p4 / 1.0e9, p5)
            return e
        except ValueError:
            return 1e20

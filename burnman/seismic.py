...
class SeismicTable(Seismic1DModel):
    ...
    def internal_depth_list(self, mindepth=0., maxdepth=1.e10):
        depths = np.array([self.table_depth[x] for x in range(len(self.table_depth)) if self.table_depth[
                          x] >= mindepth and self.table_depth[x] <= maxdepth])
    ...
    def depth(self, pressure):
        if pressure > max(self.table_pressure) or pressure < min(
                self.table_pressure):
...
class Slow(SeismicTable):
    ...
    def __init__(self):
        ...
        self.table_pressure = table[:, 2]
        self.table_density = table[:, 3]
        self.table_vp = np.interp(self.table_depth, table3[:, 0][
                                  ::-1], table3[:, 1][::-1])
        self.table_vs = np.interp(self.table_depth, table2[:, 0][
                                  ::-1], table2[:, 1][::-1])
...
class IASP91(SeismicTable):
    ...
    def __init__(self):
        SeismicTable.__init__(self)
        # depth, radius, v_p, v_s
        table = tools.read_table("input_seismic/iasp91.txt")
        table = np.array(table)
        self.table_depth = table[:, 0]
        self.table_radius = table[:, 1]
...
class AK135(SeismicTable):
    ...
    def __init__(self):
        SeismicTable.__init__(self)
        # radius, pressure, density, v_p, v_s
        table = tools.read_table("input_seismic/ak135.txt")
        table = np.array(table)
        self.table_depth = table[:, 0]
        self.table_radius = table[:, 1]

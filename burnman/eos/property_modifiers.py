...
def _landau_excesses(pressure, temperature, params):
    ...
    if temperature < Tc:
        ...
        G = params['S_D'] * ((temperature - Tc) * Q2 +
                             params['Tc_0'] * Q2 * Q2 * Q2 / 3.) - G_disordered
        dGdP = -params['V_D'] * Q2 * (1. + 0.5 * temperature /
                                      Tc * (1. - params['Tc_0'] / Tc)) - dGdP_disordered
        dGdT = params['S_D'] * Q2 * \
...
def _bragg_williams_excesses(pressure, temperature, params):
    ...
    lnxord = lambda n, Q: np.log(1. + n * Q) + \
        n * np.log(n + Q) - (1. + n) * np.log(1. + n)
...
def _magnetic_excesses_chs(pressure, temperature, params):
    ...
    dtaudP = -(temperature * params['curie_temperature']
               [1]) / (curie_temperature * curie_temperature)
    d2taudPdT = params['curie_temperature'][1] / \
        (curie_temperature * curie_temperature)
    d2taudP2 = 2. * temperature * params['curie_temperature'][1] * params[
        'curie_temperature'][1] / (curie_temperature * curie_temperature * curie_temperature)
    ...
    dGdT = gas_constant * np.log(magnetic_moment + 1.) * \
        (f + temperature * dfdT)
    d2GdT2 = gas_constant * \

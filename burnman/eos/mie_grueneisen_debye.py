...
class MGDBase(eos.EquationOfState):
    ...
    def _thermal_shear_modulus(self, T, V, params):
        if T > 1.e-10:
            ...
            G_th = 3. / 5. * (self._thermal_bulk_modulus(T, V, params) - 6 * constants.gas_constant *
                              T * params['n'] / V * gr * debye.debye_fn(Debye_T / T))  # EQ B10
            return G_th
        else:
            return 0.
    ...
    def _debye_temperature(self, x, params):
        return params['Debye_0'] * np.exp(
            (params['grueneisen_0'] - self._grueneisen_parameter(x, params)) / params['q_0'])
    ...
    def _thermal_bulk_modulus(self, T, V, params):
        if T > 1.e-10:
            gr = self._grueneisen_parameter(params['V_0'] / V, params)
            Debye_T = self._debye_temperature(params['V_0'] / V, params)
            K_th = 3. * params['n'] * constants.gas_constant * T / V * gr * ((1. - params['q_0'] - 3. * gr) * debye.debye_fn(

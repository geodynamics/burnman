...
def cork_variables(cork, cork_P, cork_T, temperature):
    a = cork[0][0] * cork_T**(2.5) / cork_P + \
        cork[0][1] * cork_T**(1.5) / cork_P * temperature
    b = cork[1][0] * cork_T / cork_P
    c = cork[2][0] * cork_T / \
        cork_P**(1.5) + cork[2][1] / cork_P**(1.5) * temperature
    d = cork[3][0] * cork_T / \
        cork_P**(2.0) + cork[3][1] / cork_P**(2.0) * temperature
    return [a, b, c, d]
...
class CORK(eos.EquationOfState):
    ...
    def volume(self, pressure, temperature, params):
        ...
        V = constants.gas_constant * temperature / pressure + (
            cork[1] - cork[0] * constants.gas_constant * np.sqrt(temperature) / (
                (constants.gas_constant * temperature + cork[1] * pressure) * (
                    constants.gas_constant * temperature + 2. * cork[1] * pressure)) + cork[2] * np.sqrt(pressure) + cork[3] * pressure)
        return V
    ...
    def gibbs_free_energy(self, pressure, temperature, volume, params):
        ...
        intCpdT = (params['Cp'][0] * temperature + 0.5 * params['Cp'][1] * np.power(temperature, 2.) - params['Cp'][2] / temperature + 2. * params['Cp'][3]
                   * np.sqrt(temperature)) - (params['Cp'][0] * T_0 + 0.5 * params['Cp'][1] * T_0 * T_0 - params['Cp'][2] / T_0 + 2.0 * params['Cp'][3] * np.sqrt(T_0))

        intCpoverTdT = (
            params['Cp'][0] * np.log(temperature) + params['Cp'][1] * temperature - 0.5 * params['Cp'][2] / np.power(
                temperature, 2.) - 2.0 * params['Cp'][3] / np.sqrt(temperature)) - (
            params['Cp'][0] * np.log(T_0) + params['Cp'][1] * T_0 - 0.5 * params['Cp'][2] / (
                T_0 * T_0) - 2.0 * params['Cp'][3] / np.sqrt(T_0))
        ...
        return params['H_0'] + intCpdT - temperature * \
            (params['S_0'] + intCpoverTdT) + RTlnf

...
class HP_TMT(eos.EquationOfState):
    ...
    def thermal_expansivity(self, pressure, temperature, volume, params):
        ...
        alpha = params['a_0'] * (C_V / C_V0) * 1. / ((1. + b * psubpth)
                                                     * (a + (1. - a) * np.power((1 + b * psubpth), c)))
        return alpha
    ...
    def heat_capacity_p0(self, temperature, params):
        ...
        Cp = params['Cp'][0] + params['Cp'][1] * temperature + params['Cp'][2] * \
            np.power(temperature, -2.) + params['Cp'][3] * np.power(temperature, -0.5)
        return Cp
    ...
    def gibbs_free_energy(self, pressure, temperature, volume, params):
        ...
        if pressure != params['P_0']:
            intVdP = (pressure - params['P_0']) * params['V_0'] * (1. - a + (a * (np.power((1. - b * Pth),
                                                                                           1. - c) - np.power((1. + b * (psubpth)), 1. - c)) / (b * (c - 1.) * (pressure - params['P_0']))))
        else:
            intVdP = 0.
        return params['H_0'] + self.__intCpdT(temperature, params) - temperature * (
            params['S_0'] + self.__intCpoverTdT(temperature, params)) + intVdP

    def helmholtz_free_energy(self, pressure, temperature, volume, params):
        return self.gibbs_free_energy(pressure,
                                      temperature,
                                      volume,
                                      params) - pressure * self.volume(pressure,
                                                                       temperature,
                                                                       params)
    ...
    def entropy(self, pressure, temperature, volume, params):
        ...
        dintVdpdx = (params['V_0'] * params['a_0'] * params['K_0'] * a * ksi_over_ksi_0) * (np.power(
            (1. + b * (pressure - params['P_0'] - Pth)), 0. - c) - np.power((1. - b * Pth), 0. - c))
        return params['S_0'] + \
            self.__intCpoverTdT(temperature, params) + dintVdpdx
    ...
    def heat_capacity_p(self, pressure, temperature, volume, params):
        ...
        dSdT = params['V_0'] * params['K_0'] * np.power((ksi_over_ksi_0 * params['a_0']), 2.0) * (np.power(
            (1. + b * (pressure - params['P_0'] - Pth)), -1. - c) - np.power((1. + b * (-Pth)), -1. - c))
    ...
    def __intCpdT(self, temperature, params):
        ...
        """
        return (params['Cp'][0] * temperature + 0.5 * params['Cp'][1] * np.power(temperature, 2.) - params['Cp'][2] / temperature + 2. * params['Cp'][3] * np.sqrt(temperature)) - \
            (params['Cp'][0] * params['T_0'] + 0.5 * params['Cp'][1] * params['T_0'] * params['T_0'] - params['Cp'][2] / params['T_0'] + 2.0 * params['Cp'][3] * np.sqrt(params['T_0']))
    ...
    def __intCpoverTdT(self, temperature, params):
        ...
        at ambient pressure [Pa]
        """
        return (params['Cp'][0] * np.log(temperature) + params['Cp'][1] * temperature - 0.5 * params['Cp'][2] / np.power(temperature, 2.) - 2.0 * params['Cp'][3] / np.sqrt(temperature)) - (
            params['Cp'][0] * np.log(params['T_0']) + params['Cp'][1] * params['T_0'] - 0.5 * params['Cp'][2] / (params['T_0'] * params['T_0']) - 2.0 * params['Cp'][3] / np.sqrt(params['T_0']))
    ...
    def validate_parameters(self, params):
        ...
        if params['G_0'] is not float('nan') and (
                params['G_0'] < 0. or params['G_0'] > 1.e13):
            warnings.warn('Unusual value for G_0', stacklevel=2)
        if params['Gprime_0'] is not float('nan') and (
                params['Gprime_0'] < -5. or params['Gprime_0'] > 10.):

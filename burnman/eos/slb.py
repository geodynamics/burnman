...
def _grueneisen_parameter_fast(V_0, volume, gruen_0, q_0):
    ...
    a2_iikk = -12. * gruen_0 + 36. * gruen_0 * \
        gruen_0 - 18. * q_0 * gruen_0  # EQ 47
...
def _delta_pressure(
    ...
    E_th = debye.thermal_energy(
        temperature,
        debye_temperature,
        n)  # thermal energy at temperature T
    ...
    return (1. / 3.) * (pow(1. + 2. * f, 5. / 2.)) * ((b_iikk * f) +
                                                      (0.5 * b_iikkmm * f * f)) + gr * (E_th - E_th_ref) / x - pressure # EQ 21
...
class SLBBase(eos.EquationOfState):
    ...
    def _debye_temperature(self, x, params):
        ...
        a2_iikk = -12. * params['grueneisen_0'] + 36. * pow(
            params['grueneisen_0'],
            2.) - 18. * params['q_0'] * params['grueneisen_0']  # EQ 47
    ...
    def _isotropic_eta_s(self, x, params):
        ...
        eta_s = - gr - (1. / 2. * pow(nu_o_nu0_sq, -1.)
                        * pow((2. * f) + 1., 2.) * a2_s)
        return eta_s
    ...
    def pressure(self, temperature, volume, params):
        ...
        P = (1. / 3.) * (pow(1. + 2. * f, 5. / 2.)) * ((b_iikk * f) + (0.5 *
                                                                       b_iikkmm * pow(f, 2.))) + gr * (E_th - E_th_ref) / volume # EQ 21
    ...
    def grueneisen_parameter(self, pressure, temperature, volume, params):
        ...
        """
        return _grueneisen_parameter_fast(params['V_0'], volume, params[
                                          'grueneisen_0'], params['q_0'])
    ...
    def shear_modulus(self, pressure, temperature, volume, params):
        ...
        if self.order == 2:
            return bm.shear_modulus_second_order(
                volume, params) - eta_s * (E_th - E_th_ref) / volume
        elif self.order == 3:
            return bm.shear_modulus_third_order(
                volume, params) - eta_s * (E_th - E_th_ref) / volume
        else:
    ...
    def internal_energy(self, pressure, temperature, volume, params):
        ...
        """
        return self.helmholtz_free_energy(pressure,
                                          temperature,
                                          volume,
                                          params) + temperature * self.entropy(pressure,
                                                                               temperature,
                                                                               volume,
                                                                               params)
    ...
    def helmholtz_free_energy(self, pressure, temperature, volume, params):
        ...
        F = params['F_0'] + 0.5 * b_iikk * f * f * params['V_0'] + \
            (1. / 6.) * params['V_0'] * b_iikkmm * f * f * f + F_quasiharmonic

...
def calculate_constraints(assemblage):
    ...
    raw_c_matrix = np.zeros((n_raw_constraints, assemblage.n_endmembers))
...
def get_parameters(assemblage):
    ...
    for i, ph in enumerate(assemblage.phases):
        ...
        if isinstance(ph, SolidSolution):
            params.extend(list(assemblage.phases[i].molar_fractions[1:]))
...
def jacobian(x, assemblage, equality_constraints):
    ...
    full_hessian = np.zeros((assemblage.n_endmembers+2, assemblage.n_endmembers+2))
    for i, (type_c, eq_c) in enumerate(equality_constraints):
        if type_c == 'P':  # dP/dx
...
def phase_fraction_constraint(phase, assemblage, proportion, prm):
...
def get_equilibration_parameters(assemblage, composition):
    ...
    prm.constraint_vector, prm.constraint_matrix, prm.raw_constraint_matrix = calculate_constraints(assemblage)
    return prm

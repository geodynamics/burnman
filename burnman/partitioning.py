...
def weight_pct_to_mol(element, amount):
    ...
    return amount * lower_mantle_mass / \
        molar_mass[element] * constants.Avogadro
...
def calculate_partition_coefficient(
        ...
        temperature,
        components,
        initial_distribution_coefficient):
    ...
    rs = ((25.e9 - pressure) * (delV) / (constants.gas_constant * temperature)
          ) + np.log(Kd_0)  # eq 5 Nakajima et al 2012, JGR. Solved for ln(K(P,T,X))
    ...
    b = (-1. + frac_mol_FeO - (frac_mol_FeO * K) + frac_mol_SiO2 -
         (frac_mol_SiO2 * K) + np.sqrt(num_to_sqrt)) / (2. * frac_mol_SiO2 * (1. - K))

    a = b / (((1. - b) * K) + b)

    # a is partition coefficient array with P for fp, b is pcarray for pv
    return (a, b)

        ...
        if len(line) != 0:
            if process==1:
                ...
                if n==1:
                    ...
                    # K_0, K_prime, Debye_0, grueneisen_0, q_0, eta_s0, [configurational_and_magnetic_entropy]
                    data.extend([ float(line[2]), float(line[5]), float(line[8]),
                    ...
                    accurate_values[name] = [data[0]/1.e3, data[1]*10., data[2]/1.e4, data[3],
...
for line in landau:
    ...
    if phase != '#':
        landau_params[phase] = [float(line[1]), float(line[2])*1.e-6, float(line[3])]

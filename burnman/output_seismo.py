...
def write_axisem_input(layers, modelname='burnmanmodel_foraxisem', axisem_ref = 'axisem_prem_ani_noocean.txt' , plotting = False):
    """
    ...
    Currently this function will not honor internal discontinuities within a layer, only discontinuities between layers.
    ...
    for layer in layers:
        # Cutting out range to input in Axisem reference file, and adding in model values at the top and bottom of Layer.
        ...
        layer_vp, layer_vs, layer_density = layer.evaluate(['v_p', 'v_s', 'density'], radlist = table[i_min:i_max,0], radius_planet = np.max(table[:,0]))
...
def write_mineos_input(layers, modelname='burnmanmodel_formineos', mineos_ref='mineos_prem_noocean.txt', plotting=False):
    """
    Writing velocities and densities to Mineos (https://geodynamics.org/cig/software/mineos/) input file
    Note:
        - Currently, it only honors the discontinuities already in the synthetic input file, so it is best to only replace certain layers with burnman values

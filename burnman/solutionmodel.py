...
class SolutionModel(object):
    ...
    def excess_gibbs_free_energy(self, pressure, temperature, molar_fractions):
        ...
        """
        return np.dot(np.array(molar_fractions), self.excess_partial_gibbs_free_energies(
            ...
            for occ in range(self.n_occupancies):
                if self.endmember_occupancies[e][occ] > 1e-10:
                    activities[e] = activities[e] * np.power(
                        ...
                        self.endmember_occupancies[e][occ] * self.site_multiplicities[occ])
            normalisation_constant = np.exp(
                self.endmember_configurational_entropies[e] /
                constants.gas_constant)
...
class AsymmetricRegularSolution (IdealSolution):
    ...
    def __init__(self, endmembers, alphas, enthalpy_interaction,
        ...
        for i in range(self.n_endmembers):
            for j in range(i + 1, self.n_endmembers):
                self.Wh[i][j] = 2. * enthalpy_interaction[i][j -
                                                             i - 1] / (self.alpha[i] + self.alpha[j])

        if entropy_interaction is not None:
            for i in range(self.n_endmembers):
                for j in range(i + 1, self.n_endmembers):
                    self.Ws[i][j] = 2. * entropy_interaction[i][j -
                                                                i - 1] / (self.alpha[i] + self.alpha[j])

        if volume_interaction is not None:
            for i in range(self.n_endmembers):
                for j in range(i + 1, self.n_endmembers):
                    self.Wv[i][j] = 2. * volume_interaction[i][j -
                                                               i - 1] / (self.alpha[i] + self.alpha[j])
    ...
    def _non_ideal_excess_partial_gibbs(
            self, pressure, temperature, molar_fractions):
    ...
    def excess_volume(self, pressure, temperature, molar_fractions):
        phi = self._phi(molar_fractions)
        V_excess = np.dot(self.alpha.T, molar_fractions) * \
            np.dot(phi.T, np.dot(self.Wv, phi))
        return V_excess
        ...
        S_conf = -constants.gas_constant * \
            np.dot(
                IdealSolution._log_ideal_activities(
                    ...
                    molar_fractions),
                molar_fractions)
        S_excess = np.dot(self.alpha.T, molar_fractions) * \
            np.dot(phi.T, np.dot(self.Ws, phi))
        return S_conf + S_excess

    def excess_enthalpy(self, pressure, temperature, molar_fractions):
        phi = self._phi(molar_fractions)
        H_excess = np.dot(self.alpha.T, molar_fractions) * \
            np.dot(phi.T, np.dot(self.Wh, phi))
        return H_excess + pressure * \
            self.excess_volume(pressure, temperature, molar_fractions)
...
class SymmetricRegularSolution (AsymmetricRegularSolution):
    ...
    def __init__(self, endmembers, enthalpy_interaction,
                 volume_interaction=None, entropy_interaction=None):
            ...
            for i in range(n):
                if i != l:
                    ...
                    for j in range(i + 1, n):
                        if j != l:
                            val += molar_fractions[i] * molar_fractions[j] * (W[i][j] * (molar_fractions[i] - molar_fractions[
                                                                              j] - 0.5) + W[j][i] * (molar_fractions[j] - molar_fractions[i] - 0.5))
            RTlny[l] = val
        return RTlny

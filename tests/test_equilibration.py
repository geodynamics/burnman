...
class equilibration(BurnManTest):
    ...
    def test_univariant_line(self):
        ...
        pressures = np.linspace(1.e5, 1.e9, 11)

        assemblage = burnman.Composite([andalusite, kyanite])
        equality_constraints = [('P', pressures),
                                ('phase_fraction',
                                 (andalusite, np.array([0.0])))]
        sols, prm = equilibrate(composition, assemblage,
                                equality_constraints)

        Ts = [sol.assemblage.temperature for sol in sols]
        Ts_ref = [464.95783870074905, 544.0828494722284, 623.6421086627697,
                  703.7607424193357, 784.4975351197959, 865.8789417005959,
                  947.9152419887187, 1030.608906228574, 1113.95923718214,
                  1197.9651099824446, 1282.6266945380332]
    ...
    def test_invariant(self):
        ...
        self.assertArraysAlmostEqual(sol.x,
                                     [4.30671426e+08, 8.09342875e+02,
                                      0., 1., 0.])
    ...
    def test_univariant_line_fo(self):
        ...
        temperatures = np.linspace(300., 2000., 11)

        assemblage = burnman.Composite(
            [forsterite, periclase, bridgmanite])
        equality_constraints = [('T', temperatures),
                                ('phase_fraction',
                                (forsterite, np.array([0.0])))]
        sols, prm = equilibrate(composition, assemblage,
                                equality_constraints)

        Ps = np.array([sol.assemblage.pressure for sol in sols])
        Ps_ref = [17405868549, 17657896452,
                  17941848050, 18231743600,
                  18517274303, 18793435710,
                  19057314295, 19306923194,
                  19540715743, 19757356711, 19955602937]
    ...
    def test_ol_wad_eqm(self):
        ...
        sol, prm = equilibrate(composition, assemblage, equality_constraints)
        self.assertArraysAlmostEqual([assemblage.temperature,
                                      ol.molar_fractions[1],
                                      wad.molar_fractions[1]],
